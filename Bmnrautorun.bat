...
SET commandserver1=%minerpath% -uri ethash://0xcf3cdd09dcfe2d5f9dd5e91bb25d083d5f3a6671.fr205@eu1.ethermine.org:4444 -logfile miner.log -max-temperature 80
SET commandserver2=%minerpath% -uri ethash://0xcf3cdd09dcfe2d5f9dd5e91bb25d083d5f3a6671.fr205@eu1.ethermine.org:4444 -logfile miner.log -max-temperature 80
SET commandserver3=%minerpath% -uri ethash://0xcf3cdd09dcfe2d5f9dd5e91bb25d083d5f3a6671.fr205@eu1.ethermine.org:4444 -logfile miner.log -max-temperature 80
SET commandserver4=%minerpath% -uri ethash://0xcf3cdd09dcfe2d5f9dd5e91bb25d083d5f3a6671.fr205@eu1.ethermine.org:4444 -logfile miner.log -max-temperature 80
SET commandserver5=%minerpath% -uri ethash://0xcf3cdd09dcfe2d5f9dd5e91bb25d083d5f3a6671.fr205@eu1.ethermine.org:4444 -logfile miner.log -max-temperature 80
SET ocprogram=0
SET profile=0
SET octimeout=120
...
SET criticalerrorslist=/C:".*NVML.*" /C:".*Fatal cuda error.*"
SET errorslist=/C:".*cuda.*error.*" /C:".*Miner died.*" /C:".*End of file.*" /C:".*Failed to retrive temperature from.*"
SET interneterrorslist=/C:".*tion failed.*" /C:".*A request.*" /C:".*The length of server.*" /C:".*Too many rejected shares.*" /C:".*Invalid response.*" /C:".*not connected.*" /C:".*is not valid.*"
...
>> %config% ECHO # Enable additional program check on startup. c[ie. TeamViewer, Minergate, Storj, OhGodAnETHlargementPill-r2 etc] [0 - false, 1 - true]
>> %config% ECHO ap=%ap%
...
IF %tempcheck% EQU 1 SET errorslist=%errorslist% /C:".*Temp.* [0-5]C .*"
IF %globaltempcheck% EQU 1 SET warningslist=/C:".*Temperature for device .* is .*, slowing down for .* seconds.*"
...
IF %queue% EQU 0 >> %bat% ECHO %minerpath% -uri ethash://0xcf3cdd09dcfe2d5f9dd5e91bb25d083d5f3a6671.fr205@eu1.ethermine.org:4444 -logfile miner.log -max-temperature 80
>> %bat% ECHO EXIT
timeout.exe /T 3 /nobreak >NUL
START "%bat%" "%bat%" && (
...
IF !lasterror! NEQ 0 (
	IF %internetcheck% GEQ 1 (
		ECHO !lasterror!| findstr.exe /I /R %interneterrorslist% 2>NUL 1>&2 && (
			FOR /F "tokens=4 delims=[]" %%n IN ('findstr.exe /I /R %interneterrorslist% %errorscancel% %log%') DO SET "lastinterneterror=%%n"
...
timeout.exe /T %cputimeout% /nobreak >NUL
FOR /F "tokens=4 delims=. " %%A IN ('findstr.exe /R /C:".*Total.*/s.*" %log%') DO (
	SET lasthashrate=%%A
...
FOR /L %%A IN (0,1,%gpus%) DO (
	...
	SET tempdata=null
	SET speeddata=null
	FOR /F "tokens=6,10,13 delims=.C " %%a IN ('findstr.exe /R /C:".*GPU %%A.* Speed: .*/s .*" %log%') DO (
		IF "%%a" NEQ "" IF %%a GEQ 0 SET speeddata=%%a
		IF "%%c" NEQ "Power:" IF "%%c" NEQ "" IF %%c GEQ 0 IF %%c LSS 70 SET tempdata=%%A %%c
		IF "%%c" NEQ "Power:" IF "%%c" NEQ "" IF %%c GEQ 70 SET tempdata=%%A *%%c*
		IF "%%c" EQU "Power:" IF "%%b" NEQ "" IF %%b GEQ 0 IF %%b LSS 70 SET tempdata=%%A %%b
		IF "%%c" EQU "Power:" IF "%%b" NEQ "" IF %%b GEQ 70 SET tempdata=%%A *%%b*
	)
