...
IF NOT EXIST "%log%" (
	...
	findstr.exe /R /C:".*%minerprocess% -epool.*-tstop.*-logfile %log%.*" %bat% 2>NUL 1>&2 || (
		CALL :inform "1" "false" "Ensure *%minerpath% -epool -tstop -logfile %log%* options added to the miners command line in this order." "Ensure %minerpath% -epool -tstop -logfile %log% options added to the miners command line in this order." "2"
	)
...
FOR /F "delims=" %%N IN ('findstr.exe /I /R %criticalerrorslist% %errorslist% %warningslist% %interneterrorslist% %log% ^| findstr.exe /V /R /C:".*DevFee.*" /C:".*mining mode is enabled.*" /C:".*DCR.*" /C:".*SC.*" /C:".*LBC.*" /C:".*PASC.*" /C:".*B2S.*" /C:".*KC.*"') DO (
	SET "lasterror=%%N"
...
IF !lasterror! NEQ 0 (
	IF %internetcheck% GEQ 1 (
		ECHO !lasterror!| findstr.exe /I /R %interneterrorslist% 2>NUL 1>&2 && (
			FOR /F "delims=" %%n IN ('findstr.exe /I /R %interneterrorslist% %errorscancel% %log% ^| findstr.exe /V /R /C:".*DevFee.*" /C:".*mining mode is enabled.*" /C:".*DCR.*" /C:".*SC.*" /C:".*LBC.*" /C:".*PASC.*" /C:".*B2S.*" /C:".*KC.*"') DO SET "lastinterneterror=%%n"
...
timeout.exe /T %cputimeout% /nobreak >NUL
FOR /F "delims=" %%A IN ('findstr.exe /R /C:".*GPU.* .*/s.*" %log% ^| findstr.exe /V /R /C:".*DevFee.*" /C:".*mining mode is enabled.*" /C:".*DCR.*" /C:".*SC.*" /C:".*LBC.*" /C:".*PASC.*" /C:".*B2S.*" /C:".*KC.*"') DO SET curspeedcache=%%A
