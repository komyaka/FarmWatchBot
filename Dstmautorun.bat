...
SET commandserver1=%minerpath% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr205 --pass x --logfile --time
SET commandserver2=%minerpath% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr205 --pass x --logfile --time
SET commandserver3=%minerpath% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr205 --pass x --logfile --time
SET commandserver4=%minerpath% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr205 --pass x --logfile --time
SET commandserver5=%minerpath% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr205 --pass x --logfile --time
SET ocprogram=0
SET profile=0
SET octimeout=120
...
IF %queue% EQU 0 >> %bat% ECHO %minerpath% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr205 --pass x --logfile --time
>> %bat% ECHO EXIT
timeout.exe /T 3 /nobreak >NUL
START "%bat%" "%bat%" && (
...
IF NOT EXIST "%log%" (
	findstr.exe /R /C:".*--logfile.*" %bat% 2>NUL 1>&2 || (
		CALL :inform "1" "false" "%log% is missing. Ensure *--logfile* option is added to the miners command line in *%config%* file." "%log% is missing. Ensure --logfile option is added to the miners command line in %config% file." "2"
		PAUSE
		EXIT
	)
...
IF %firstrun% EQU 0 (
	timeout.exe /T %cputimeout% /nobreak >NUL
	FOR /F "delims=" %%A IN ('findstr.exe /R /C:".*GPU.*\+.*PCI:.*" %log%') DO SET /A gpucount+=1
	FOR /F "delims=" %%A IN ('findstr.exe /R /C:".*GPU.*PCI:.*" %log%') DO SET /A totalgpucount+=1
