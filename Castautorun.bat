...
SET commandserver1=%minerpath% -S xmr-eu1.nanopool.org:14444 -u 4JUdGzvrMFDWrUUwY3toJATSeNwjn54LkCnKBPRzDuhzi5vSepHfUckJNxRL2gjkNrSqtCoRUrEDAgRwsQvVCjZbS5VF8ypv8VF3GUiS1J.fr205 -p x --log miner.log
SET commandserver2=%minerpath% -S xmr-eu1.nanopool.org:14444 -u 4JUdGzvrMFDWrUUwY3toJATSeNwjn54LkCnKBPRzDuhzi5vSepHfUckJNxRL2gjkNrSqtCoRUrEDAgRwsQvVCjZbS5VF8ypv8VF3GUiS1J.fr205 -p x --log miner.log
SET commandserver3=%minerpath% -S xmr-eu1.nanopool.org:14444 -u 4JUdGzvrMFDWrUUwY3toJATSeNwjn54LkCnKBPRzDuhzi5vSepHfUckJNxRL2gjkNrSqtCoRUrEDAgRwsQvVCjZbS5VF8ypv8VF3GUiS1J.fr205 -p x --log miner.log
SET commandserver4=%minerpath% -S xmr-eu1.nanopool.org:14444 -u 4JUdGzvrMFDWrUUwY3toJATSeNwjn54LkCnKBPRzDuhzi5vSepHfUckJNxRL2gjkNrSqtCoRUrEDAgRwsQvVCjZbS5VF8ypv8VF3GUiS1J.fr205 -p x --log miner.log
SET commandserver5=%minerpath% -S xmr-eu1.nanopool.org:14444 -u 4JUdGzvrMFDWrUUwY3toJATSeNwjn54LkCnKBPRzDuhzi5vSepHfUckJNxRL2gjkNrSqtCoRUrEDAgRwsQvVCjZbS5VF8ypv8VF3GUiS1J.fr205 -p x --log miner.log
SET ocprogram=0
SET profile=0
SET octimeout=120
...
SET errorscancel=/C:".*New job received.*"
SET criticalerrorslist=/C:".*Restarting failed.*"
SET errorslist=/C:".*GPU .* hangs.*"
SET interneterrorslist=/C:".*SOCKET ERROR.*" /C:".*Connecting to Pool failed.*"
...
IF %queue% EQU 0 >> %bat% ECHO %minerpath% -S xmr-eu1.nanopool.org:14444 -u 4JUdGzvrMFDWrUUwY3toJATSeNwjn54LkCnKBPRzDuhzi5vSepHfUckJNxRL2gjkNrSqtCoRUrEDAgRwsQvVCjZbS5VF8ypv8VF3GUiS1J.fr205 -p x --log miner.log
>> %bat% ECHO EXIT
timeout.exe /T 3 /nobreak >NUL
START "%bat%" "%bat%" && (
...
IF NOT EXIST "%log%" (
	findstr.exe /R /C:".*--log %log%.*" %bat% 2>NUL 1>&2 || (
		CALL :inform "1" "false" "%log% is missing. Ensure *--log %log%* option is added to the miners command line in *%config%* file." "%log% is missing. Ensure --log %log% option is added to the miners command line in %config% file." "2"
		PAUSE
		EXIT
	)
...
IF %firstrun% EQU 0 (
	timeout.exe /T %cputimeout% /nobreak >NUL
	FOR /F "delims=" %%A IN ('findstr.exe /R /C:".*GPU.*:.*Memory.*" %log%') DO SET /A gpucount+=1
...
FOR /L %%A IN (0,1,%gpus%) DO (
	...
	SET speeddata=null
	SET tempdata=null
	FOR /F "tokens=3,5 delims=.шCFanRPMGU| " %%a IN ('findstr.exe /R /C:".*GPU%%A.*/s.*" %log%') DO (
		IF "%%a" NEQ "" IF %%a GEQ 0 (
			IF %%a LSS 70 SET tempdata=%%A %%a
			IF %%a GEQ 70 SET tempdata=%%A *%%a*
		)
